...
namespace NGraphics
{
	public class SvgReader
	{
		...
		void AddElement(IList<Element> list, XElement e, Pen inheritPen, Brush inheritBrush)
		{
			...
			//
			Element r = null;
			Pen pen = inheritPen?.Clone();
			Brush brush = inheritBrush?.Clone();

			var styleValues = e.Attributes().ToDictionary(k => k.Name.LocalName, v => v.Value);
			var style = ReadString(e.Attribute("style"));
			if (!string.IsNullOrWhiteSpace(style))
			{
				ParseStyle(style, styleValues);
			}
			ApplyStyle(styleValues, ref pen, ref brush);
			...
			switch (e.Name.LocalName)
			{
				...
				case "pgf":
					{
						Debug.WriteLine("Ignoring pgf element" + id != null ? ": '" + id + "'" : "");
					}
					break;
		...
		Dictionary<string, string> ParseStyle(string style, Dictionary<string, string> styles = null)
		{
		...
		void ApplyStyle(Dictionary<string, string> style, ref Pen pen, ref Brush brush)
		{
			...
			else if (!string.IsNullOrEmpty(stroke))
			{
				...
				{
					if (pen.Color.Alpha > 1 - double.Epsilon)
						pen.Color = color;
					else
			...
			if (brush != null)
			{
				...
				if (!string.IsNullOrWhiteSpace(fillOpacity))
				{
					brush = brush?.WithAlpha(ReadNumber(fillOpacity));
				}
			}
		}
		...
		static readonly Regex unitRe = new Regex("px|pt|em|ex|pc|cm|mm|in");
		static readonly Regex percRe = new Regex("%");
