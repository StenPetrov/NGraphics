    ...
    public class Text : Element
    {
        ...
        protected override void DrawElement(ICanvas canvas)
        {
            foreach (var s in Spans)
            {
                if (s.Position != null)
                {
                    var textMetrix = canvas.MeasureText(s.Text, s.Font ?? Font);
                    canvas.DrawText(s.Text, new Rect(s.Position.Value, textMetrix.Size), s.Font ?? Font, TextAlignment.Left, Pen, Brush);
                }
                else
                {
